...
test('update troca engine, role, model e descrição sem tocar no prompt', async (t) => {
  ...
  const report = await updateAgentCommand('dev',
    {
      cwd, engine: 'codex', role: 'architect', model: 'gpt-6-astra',
      reasoningEffort: 'xhigh', description: 'Arquiteto chefe',
    });
  assert.deepEqual(report.updated, ['engine', 'role', 'description', 'model', 'reasoningEffort']);
...
test('clear-model remove a chave; nada para atualizar e inválidos falham sem gravar', async (t) => {
  const cwd = await fixture(t);
  await updateAgentCommand('dev', { cwd, model: 'x', reasoningEffort: 'high' });
  const cleared = await updateAgentCommand('dev', {
    cwd, clearModel: true, clearReasoningEffort: true,
  });
  assert.deepEqual(cleared.updated, ['model', 'reasoningEffort']);
  assert.equal((await loadAgent('dev', { cwd })).model, undefined);
  assert.equal((await loadAgent('dev', { cwd })).reasoningEffort, undefined);
  ...
  await assert.rejects(
    updateAgentCommand('dev', { cwd, reasoningEffort: 'high', clearReasoningEffort: true }),
    /não ambos/,
  );
  await assert.rejects(updateAgentCommand('dev', { cwd, reasoningEffort: 'max' }), /reasoningEffort/);
...
test('CLI agent update troca o modelo do perfil', async (t) => {
  ...
  const report = JSON.parse((await run([
    'agent', 'update', 'dev', '--model', 'opencode/big-pickle', '--reasoning-effort', 'medium',
  ])).stdout);
  assert.deepEqual(report.updated, ['model', 'reasoningEffort']);
  const shown = JSON.parse((await run(['agent', 'show', 'dev'])).stdout);
  assert.equal(shown.model, 'opencode/big-pickle');
  assert.equal(shown.reasoningEffort, 'medium');
