...
export async function routeTask(task, { cwd = process.cwd(), skills = [] } = {}) {
  ...
  for (const { agent } of entries) {
    ...
    candidates.push({ agent: agent.name, engine: agent.engine, model: agent.model ?? null, score, reasons });
  }
  ...
  return {
    selected: { agent: config.agents.default, engine: resolved.adapter.command, model: null, score: 0, reasons: ['sem correspondência; usando agents.default'] },
